...
await runTest("Test 1: Successful Generation", async () => {
  ...
  if (response.status === 200) {
    ...
    logTest("Successful Generation", "FAIL", `Expected 200, got ${response.status}: ${JSON.stringify(data)}`);
  }
});
...
await runTest("Test 2: Validation Error - Text Too Short", async () => {
  ...
  if (response.status === 400 && data.error?.code === "VALIDATION_ERROR") {
    logTest("Text Too Short Validation", "PASS", `Correctly rejected: ${data.error.details?.reason}`);
  } else {
    logTest("Text Too Short Validation", "FAIL", `Expected 400 VALIDATION_ERROR, got ${response.status}`);
  }
});
...
await runTest("Test 3: Validation Error - Invalid UUID", async () => {
  ...
  if (response.status === 400 && data.error?.code === "VALIDATION_ERROR") {
    logTest("Invalid UUID Validation", "PASS", `Correctly rejected invalid UUID`);
  } else {
    logTest("Invalid UUID Validation", "FAIL", `Expected 400 VALIDATION_ERROR, got ${response.status}`);
  }
});
...
await runTest("Test 4: Unauthorized - Missing Token", async () => {
  ...
  if (response.status === 401 && data.error?.code === "UNAUTHORIZED") {
    logTest("Missing Token Auth Check", "PASS", `Correctly rejected: ${data.error.message}`);
  } else {
    logTest("Missing Token Auth Check", "FAIL", `Expected 401 UNAUTHORIZED, got ${response.status}`);
  }
});
...
await runTest("Test 5: Unauthorized - Invalid Token", async () => {
  ...
  if (response.status === 401 && data.error?.code === "UNAUTHORIZED") {
    logTest("Invalid Token Auth Check", "PASS", `Correctly rejected invalid token`);
  } else {
    logTest("Invalid Token Auth Check", "FAIL", `Expected 401 UNAUTHORIZED, got ${response.status}`);
  }
});
...
await runTest("Test 6: Not Found - Deck Does Not Exist", async () => {
  ...
  if (response.status === 404 && data.error?.code === "NOT_FOUND") {
    logTest("Deck Not Found Check", "PASS", `Correctly returned 404: ${data.error.message}`);
  } else {
    logTest("Deck Not Found Check", "FAIL", `Expected 404 NOT_FOUND, got ${response.status}`);
  }
});
