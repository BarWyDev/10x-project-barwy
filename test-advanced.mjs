...
await runTest('Test 7: Maximum Text Length (5000 chars)', async () => {
  const longText = 'A'.repeat(50) + ' Photosynthesis is the process by which plants convert light energy into chemical energy. ' + 'B'.repeat(4800);
  ...
  if (response.status === 200) {
    logTest('Max Text Length', 'PASS', 
      `Generated ${data.proposals.length} proposals from 5000 chars`);
  } else {
    logTest('Max Text Length', 'FAIL', 
...
await runTest('Test 8: Text Over Limit (5001 chars)', async () => {
  ...
  if (response.status === 400 && data.error?.code === 'VALIDATION_ERROR') {
    logTest('Text Over Limit', 'PASS', 
      `Correctly rejected text over 5000 chars`);
  } else {
    logTest('Text Over Limit', 'FAIL', 
...
await runTest('Test 9: Text Exactly 50 Characters (Boundary)', async () => {
  ...
  if (response.status === 200 || response.status === 422) {
    ...
    logTest('Boundary 50 Chars', 'PASS', 
      `Accepted 50-char text (status: ${response.status})`);
  } else {
    logTest('Boundary 50 Chars', 'FAIL', 
...
await runTest('Test 10: Whitespace Trimming', async () => {
  ...
  if (response.status === 200) {
    logTest('Whitespace Trimming', 'PASS', 
      `Correctly handled text with leading/trailing spaces`);
  } else {
    logTest('Whitespace Trimming', 'FAIL', 
      `Expected 200, got ${response.status}`);
  }
});
...
await runTest('Test 11: Empty JSON Body', async () => {
  ...
  if (response.status === 400 && data.error?.code === 'VALIDATION_ERROR') {
    logTest('Empty Body Validation', 'PASS', 
      `Correctly rejected empty request body`);
  } else {
    logTest('Empty Body Validation', 'FAIL', 
      `Expected 400 VALIDATION_ERROR, got ${response.status}`);
  }
});
...
await runTest('Test 12: Malformed JSON', async () => {
  ...
  if (response.status === 400 && data.error?.code === 'VALIDATION_ERROR') {
    logTest('Malformed JSON', 'PASS', 
      `Correctly rejected malformed JSON`);
  } else {
    logTest('Malformed JSON', 'FAIL', 
      `Expected 400 VALIDATION_ERROR, got ${response.status}`);
  }
});
  ...
  if (response.status === 200) {
    ...
    if (duration < 30000) {
      logTest('Response Time', 'PASS', 
        `Completed in ${durationSeconds}s (under 30s timeout)`);
    } else {
      logTest('Response Time', 'FAIL', 
        `Took ${durationSeconds}s (over 30s timeout)`);
    }
  } else {
    logTest('Response Time', 'FAIL', 
