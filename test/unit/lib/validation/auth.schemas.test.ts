  ...
  describe("loginSchema", () => {
    describe("valid scenarios", () => {
      ...
      it("should accept special characters in password", () => {
        // Arrange
        const specialPasswords = ["Pass@word123!", "p@ssw#rd$", "test-pass_word.123"];
  ...
  describe("registerSchema", () => {
    ...
    describe("password complexity validation", () => {
        ...
        shortPasswords.forEach((password) => {
          ...
          if (!result.success) {
            const passwordError = result.error.issues.find(
              (i) => i.path.includes("password") && i.message.includes("co najmniej 8")
            );
            expect(passwordError).toBeDefined();
          }
        });
      ...
      it("should reject password without lowercase letter", () => {
        // Arrange
        const invalidPasswords = ["PASSWORD123", "SECURE1PASS", "TEST12345"];
        ...
        invalidPasswords.forEach((password) => {
          ...
          if (!result.success) {
            const passwordError = result.error.issues.find(
              (i) => i.path.includes("password") && i.message.includes("małą literę")
            );
      ...
      it("should reject password without uppercase letter", () => {
        ...
        invalidPasswords.forEach((password) => {
          ...
          if (!result.success) {
            const passwordError = result.error.issues.find(
              (i) => i.path.includes("password") && i.message.includes("dużą literę")
            );
            expect(passwordError?.message).toBe("Hasło musi zawierać małą literę, dużą literę i cyfrę");
          }
        });
      ...
      it("should reject password without digit", () => {
        // Arrange
        const invalidPasswords = ["PasswordOnly", "SecurePass", "TestPassword"];
        ...
        invalidPasswords.forEach((password) => {
          ...
          if (!result.success) {
            const passwordError = result.error.issues.find(
              (i) => i.path.includes("password") && i.message.includes("cyfrę")
            );
            expect(passwordError?.message).toBe("Hasło musi zawierać małą literę, dużą literę i cyfrę");
          }
        });
    ...
    describe("password confirmation validation", () => {
      ...
      it("should reject empty confirmPassword", () => {
        ...
        if (!result.success) {
          const confirmError = result.error.issues.find((i) => i.message.includes("Potwierdzenie hasła jest wymagane"));
  ...
  describe("forgotPasswordSchema", () => {
    describe("valid scenarios", () => {
      ...
      it("should accept various valid email formats", () => {
        // Arrange
        const validEmails = ["user@test.com", "admin+tag@example.co.uk", "first.last@subdomain.example.com"];
    ...
    describe("email validation", () => {
      ...
      it("should reject invalid email formats", () => {
        // Arrange
        const invalidEmails = ["not-an-email", "@example.com", "user@", "user domain@example.com"];
  ...
  describe("resetPasswordSchema", () => {
    ...
    describe("password complexity validation", () => {
      it("should reject password shorter than 8 characters", () => {
        ...
        if (!result.success) {
          const passwordError = result.error.issues.find((i) => i.message.includes("co najmniej 8"));
          expect(passwordError).toBeDefined();
        }
      });
  ...
  describe("cross-schema validation", () => {
    ...
    it("registerSchema and resetPasswordSchema should have same password requirements", () => {
      ...
      testPasswords.forEach(({ pass, shouldPass }) => {
        ...
        expect(registerResult.success, `Register: password "${pass}" should ${shouldPass ? "pass" : "fail"}`).toBe(
          shouldPass
        );
        expect(resetResult.success, `Reset: password "${pass}" should ${shouldPass ? "pass" : "fail"}`).toBe(
          shouldPass
        );
      });
    });
