...
export function EditableFlashcardProposal({ proposal, index, onChange, onDelete }: EditableFlashcardProposalProps) {
    ...
    <Card className={hasErrors ? "border-red-300" : undefined}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-gray-600">Fiszka {index + 1}</span>
          {proposal.isEdited && (
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <CharacterCounter current={proposal.front_content.length} max={FRONT_MAX} />
          </div>
          <Input
            ...
            aria-describedby={proposal.errors.front_content ? `front-error-${proposal.id}` : undefined}
          />
          {proposal.errors.front_content && (
            <p id={`front-error-${proposal.id}`} className="text-xs text-red-600">
              {proposal.errors.front_content}
            </p>
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <CharacterCounter current={proposal.back_content.length} max={BACK_MAX} />
          </div>
          <Textarea
            id={`back-${proposal.id}`}
            ...
            className={`min-h-[100px] resize-y ${proposal.errors.back_content ? "border-red-500" : ""}`}
            aria-invalid={!!proposal.errors.back_content}
            aria-describedby={proposal.errors.back_content ? `back-error-${proposal.id}` : undefined}
          />
          {proposal.errors.back_content && (
            <p id={`back-error-${proposal.id}`} className="text-xs text-red-600">
              {proposal.errors.back_content}
            </p>
          )}
        </div>
