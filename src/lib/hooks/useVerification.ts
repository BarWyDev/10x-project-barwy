...
export function useVerification(demoMode: boolean = false): UseVerificationReturn {
    ...
    (id: string, field: 'front_content' | 'back_content', value: string) => {
      setProposals(prev => 
        prev.map(proposal => {
          ...
          const originalValue = field === 'front_content' 
          ...
          const validation = validateFlashcardContent(
            updatedProposal.front_content,
            updatedProposal.back_content
          );
  ...
  const saveAllProposals = useCallback(
    async (deckId: string, demo: boolean = false): Promise<BatchCreateFlashcardsResponse | null> => {
