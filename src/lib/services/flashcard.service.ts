...
type FlashcardInsert = Omit<
  Flashcard,
  "id" | "created_at" | "updated_at" | "last_reviewed_at" | "due_at" | "interval" | "ease_factor" | "repetitions"
>;
...
export class FlashcardService {
  ...
  async createFlashcard(flashcardData: FlashcardInsert): Promise<Flashcard> {
    ...
    if (error || !data) {
      ...
      throw new APIError(500, "INTERNAL_ERROR", "Could not create flashcard");
    }
  ...
  async getAllFlashcards(userId: string, deckId?: string) {
    ...
    if (error) {
      ...
      throw new APIError(500, "INTERNAL_ERROR", "Could not fetch flashcards");
    }
  ...
  async updateFlashcard(
    ...
    if (error || !data) {
      ...
      throw new APIError(500, "INTERNAL_ERROR", "Could not update flashcard");
    }
  ...
  async deleteFlashcard(flashcardId: string, userId: string): Promise<void> {
    ...
    if (error) {
      ...
      throw new APIError(500, "INTERNAL_ERROR", "Could not delete flashcard");
    }
  }
}
