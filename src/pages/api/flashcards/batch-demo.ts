...
import type { BatchCreateFlashcardsResponse, ErrorResponse, ErrorCode } from "../../../types";
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const mockCreatedFlashcards = flashcards.map((flashcard, index) => ({
      id:
        `${Date.now()}${index}`.padStart(36, "0").slice(0, 8) +
        "-0000-0000-0000-" +
        `${Date.now()}${index}`.padStart(12, "0").slice(-12),
